...
public class PC extends javax.swing.JFrame {
    ...
    private void loadData() {
        ...
        model.setRowCount(0);
        model.setColumnIdentifiers(new String[] { "ID", "Model", "Spesifikasi" });
    ...
    private void btnUbah1ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnUbah1ActionPerformed
        ...
        try (Connection conn = KoneksiDB.getConnection()) {
            String sql = "UPDATE pc SET model=?, spesifikasi=? WHERE id_pc=?";
    ...
    private void btnTambah3ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnTambah3ActionPerformed
        try (Connection conn = KoneksiDB.getConnection()) {
            String sql = "INSERT INTO pc (model, spesifikasi) VALUES (?, ?)";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setString(1, model.getText());
            stmt.setString(2, jTextArea1.getText());
            stmt.executeUpdate();
    ...
    private void btnCari1ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try (Connection conn = KoneksiDB.getConnection()) {
            String sql = "SELECT * FROM pc WHERE LOWER(model) LIKE ? OR LOWER(spesifikasi) LIKE ?";
            PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    private void btnCetak1ActionPerformed(java.awt.event.ActionEvent evt) {
        try {
            String[] headers = {"Model", "Spesifikasi"};
            String filename = "laporan_pc_" + System.currentTimeMillis() + ".pdf";
            PDFGenerator.generatePDF("Data PC", headers, jTable2, filename);
    ...
    private void jTable2MouseClicked(java.awt.event.MouseEvent evt) {
        ...
        if (row != -1) {
            ...
            if (isEditing && clickedId == selectedId) {
                ...
                this.model.setText(tableModel.getValueAt(row, 1).toString());
                jTextArea1.setText(tableModel.getValueAt(row, 2).toString());
                isEditing = true;
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new PC(new Penyewaan()).setVisible(true);
            }
        });
    }
